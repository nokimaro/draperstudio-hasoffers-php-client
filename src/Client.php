...
class Client
{
    ...
    public function get($apiMethod, $parameters = [])
    {
        ...
        //[errorMessage] => everyads.  API usage exceeded rate limit.  Configured: 50/10s window; Your usage: 51.  See https://help.tune.com/hasoffers/rate-limit-error/ for guidance.
        if(isset($json_response->response->errorMessage) && stripos($json_response->response->errorMessage, 'rate limit') !== false)
        {
            $count_fails++;
            $sleep_time = $count_fails*2;
            echo 'SLEEP: '.$sleep_time.' sec ('.$apiMethod.' / errorMessage: '.$json_response->response->errorMessage.')'."\n";
            sleep($sleep_time);
